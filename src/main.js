...
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1240, height: 820, minWidth: 760, minHeight: 620,
    backgroundColor: '#0e1116', title: 'PageSmith PDF', icon: path.join(__dirname, 'pagesmith-mark.ico'), autoHideMenuBar: true,
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('get-pdf-info', async (_event, filePath) => ({ pageCount: (await PDFDocument.load(await fs.readFile(filePath))).getPageCount() }));
  ipcMain.handle('get-pdf-thumbnails', (_event, filePath, pageNumbers) => getPdfThumbnails(filePath, pageNumbers));
  ipcMain.handle('run-job', async (_event, request) => {
    try { return await jobs.runJob(request); } catch (error) { throw new Error(jobs.localiseError(error.message, request.language)); }
  });
}

if (process.env.PAGESMITH_TEST !== '1') {
  app.whenReady().then(() => { createWindow(); registerIpcHandlers(); app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createWindow(); }); });
