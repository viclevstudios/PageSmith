...
async function run() {
  ...
  const workspace = await fs.mkdtemp(
    path.join(os.tmpdir(), "pagesmith-pdf-test-"),
  );
  ...
  try {
    ...
    await createSamplePdf(sourcePdf);
    await createSamplePdf(secondPdf);

    const converted = await tools.convertToPdf([textFile], output, {
      combine: "separate",
    });
    await expectFile(converted[0]);
    assert.equal(
      await pageCount(converted[0]),
      1,
      "text conversion should create one PDF page",
    );
    const renamed = await tools.convertToPdf([textFile], output, {
      combine: "separate",
    });
    assert.match(
      path.basename(renamed[0]),
      /note \(1\)\.pdf$/,
      "existing output should receive a numbered filename by default",
    );
    await expectFile(renamed[0]);
    const merged = await tools.mergePdfs([sourcePdf, secondPdf], output);
    await expectFile(merged[0]);
    assert.equal(
      await pageCount(merged[0]),
      6,
      "merge should preserve every page",
    );
    await assert.rejects(
      () =>
        tools.runJob({
          tool: "merge",
          files: [sourcePdf, secondPdf],
          outputDir: output,
    ...
    assert.deepEqual(
      overwriteDialogs.at(-1).buttons,
      ["Overwrite", "Cancel"],
      "overwrite dialog should follow the selected language",
    );
    ...
    await Promise.all(split.map(expectFile));
    assert.deepEqual(await Promise.all(split.map(pageCount)), [1, 1, 1]);

    const reordered = await tools.reorderPdf([sourcePdf], output, "3,1,2");
    await expectFile(reordered[0]);
    assert.equal(
      await pageCount(reordered[0]),
      3,
      "reorder should keep all pages",
    );
    await assert.rejects(
      () => tools.reorderPdf([sourcePdf], output, "1,1,2"),
      /genau einmal/,
    );

    const compressed = await tools.compressPdf([sourcePdf], output, "lossless");
    await expectFile(compressed[0]);
    assert.equal(
      await pageCount(compressed[0]),
      3,
      "lossless compression should preserve pages",
    );

    const textExport = await tools.exportPdf(sourcePdf, output, "txt");
    assert.match(
      await fs.readFile(textExport[0], "utf8"),
      /Alpha page/,
      "text export should contain PDF text",
    );

    const wordExport = await tools.exportPdf(sourcePdf, output, "docx");
    await expectFile(wordExport[0]);
    console.log(
      `All PageSmith PDF core-feature tests passed in ${(performance.now() - startedAt).toFixed(0)} ms.`,
    );
  } finally {
